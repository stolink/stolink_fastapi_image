    ...
    def invoke_nova_canvas(self, prompt: str, negative_prompt: str = "") -> bytes:
        ...
        try:
            ...
            if "images" not in response_body:
                logger.error(f"Nova Canvas response missing 'images' key: {response_body}")
                if "error" in response_body:
                    raise ClientError(
                        {"Error": {"Code": "NovaCanvasError", "Message": response_body["error"]}},
                        "InvokeModel"
                    )
                raise ValueError(f"Invalid response from Nova Canvas: {response_body}")
